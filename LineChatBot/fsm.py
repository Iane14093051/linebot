...
class TocMachine(GraphMachine):
    ...
    def is_going_to_contact_information(self, event):
        text = event.message.text
        if text == '聯絡我們':
            return True
        return False
    ...
    def on_enter_contact_information(self, event):
        send_text_message(event.reply_token, '官方帳號無法各別回覆訊息，若有其他問題請電洽客服專線，將有專人為您服務，謝謝您耐心等候。  客服專線: 0888-999-680 　                 服務時間: 每周一至周五 9:30 -18:00 (不含章魚哥的休假日)')
        self.go_back()
    ...
    def on_enter_location(self, event):
        ...
        line_bot_api.reply_message(
            ...
            title='my location',
                ...
                '''
        linebot.models.send_messages.LocationSendMessage( title='my location',  
        address='蟹堡王', latitude=32.6948660,
        longitude=-162.0703120, quick_reply=None,
        sender=None, **kwargs)
    ...
    def on_enter_reserve_result(self, event):
        ...
        if x == 0:
            url = 'https://img.onl/q4QIHO'
            send_image_message(event.reply_token, url)
        else:
            url = 'https://img.onl/rROg7'
            send_image_message(event.reply_token, url)
        self.go_back()
